...
function updateClock(){
  const now=new Date();
  document.getElementById("clock").innerText= now.toLocaleDateString('th-TH')+" ⏰ "+now.toLocaleTimeString('th-TH');
}
...
const classroom=[13.769145,100.560838];
...
function checkIn(){
  ...
  if(!userMarker){
    showLog("ไม่สามารถระบุตำแหน่งผู้ใช้ได้","error");
    return;
  }

  // คำนวณระยะทาง
  const userPos = userMarker.getLatLng();
  const classroomPos = L.latLng(classroom[0], classroom[1]);
  const distance = userPos.distanceTo(classroomPos);

  const MAX_DISTANCE = 50; // เมตร

  if(distance > MAX_DISTANCE){
    showLog(`❌ คุณอยู่ห่างจากห้องเรียน ${Math.round(distance)} เมตร ไม่สามารถเช็คชื่อได้`, "error");
    return;
  }
  ...
  if(!allHistory[date].some(x=>x.id===id)){
    allHistory[date].push({id:id,name:students[id],time:time,pos:[userPos.lat,userPos.lng]});
    localStorage.setItem("attendance",JSON.stringify(allHistory));
    showLog(`✔ ${students[id]} เช็คชื่อแล้ว<br>${time}`,"success");
    loadHistory(); updateChart(); addLiveMarker(id, students[id], [userPos.lat,userPos.lng]);
